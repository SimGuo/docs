...
int main()
{
    ...
    {
        temp1 = CV2SGMatrixFactory::getSGMatrix<float64_t>(images[j], CV2SG_MEMCPY);
        temp2 = flatten(temp1);
    ...
    SGMatrix<float64_t> testimage_sgmat = CV2SGMatrixFactory::getSGMatrix<float64_t>(testimage, CV2SG_MEMCPY);
    ...
    SGMatrix<float64_t>data_matrix(testimage_projected_vec.vlen, 1, false);
    ...
    float64_t distance_array[ NO_OF_EIGENFACES ];
    int min_index = 0;
    for (int i=0; i<NO_OF_EIGENFACES; ++i)
    {
